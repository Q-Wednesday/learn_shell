...
## 挂在、卸载储存设备
Linux图形界面操作最近已经能让用户很轻易地管理设备。多数情况下连上就能用。在过去（大概2004年）这都还要手动操作。<br>
管理存储设备首先要做的就是把设备添加到文件系统树中，从而允许操作系统操作设备，称之为挂载。

1. 查看已挂载的文件系统列表
<br>mount命令用于文件系统挂载，不带任何参数输入该命令会调出目前已经挂载的文件系统列表,格式是<br>
*device* on *mount_point* type *filesystem_type*(*options*)<br>
mount 原位置 目标位置
<br>进行挂载操作2.umount 原位置可以卸载设备。

2. 确定设备名称<br>
对于现在的系统，有时候设备名称的确定会比较困难。在过去，由于设备总是放在一个地方不动，所以确定设备名不难。设备命名有一些固定的模式：<br>
/dev/fd* 软盘取东西<br>
/dev/hd* 较旧系统上的IDE（或PATA）硬盘<br>
/dev/lp* 打印机设备<br>
/dev/sd* SCSI硬盘，最近的Linux系统上把所有类硬盘设备都当作SCSI硬盘<br>
/dev/sr* 光驱
...
后面部分内容由于手头没有相应的硬件设备以及目前没有需求，故跳过

# 网络
在网络连接上Linux可以说是万能的。可以建立各种网络系统及应用，包括防火墙、路由器、域名服务器、NAS(网络附加储存)盒等。
